#ifndef __included__src_vpp_build_root_build_vpp_native_vpp_vnet_ip_punt_api_json
#define __included__src_vpp_build_root_build_vpp_native_vpp_vnet_ip_punt_api_json
...
extern vapi_msg_id_t vapi_msg_id_punt_reason_dump;
...
extern vapi_msg_id_t vapi_msg_id_punt_reason_details;

#define DEFINE_VAPI_MSG_IDS_PUNT_API_JSON\
  vapi_msg_id_t vapi_msg_id_punt_socket_dump;\
  vapi_msg_id_t vapi_msg_id_punt_reason_dump;\
  ...
  vapi_msg_id_t vapi_msg_id_punt_reason_details;
...
#ifndef defined_vapi_type_punt_reason
#define defined_vapi_type_punt_reason
typedef struct __attribute__((__packed__)) {
  u32 id;
  vl_api_string_t name;
} vapi_type_punt_reason;

static inline void vapi_type_punt_reason_hton(vapi_type_punt_reason *msg)
{
  msg->id = htobe32(msg->id);
}

static inline void vapi_type_punt_reason_ntoh(vapi_type_punt_reason *msg)
{
  msg->id = be32toh(msg->id);
}
#endif
...
#ifndef defined_vapi_msg_punt_reason_details
#define defined_vapi_msg_punt_reason_details
typedef struct __attribute__ ((__packed__)) {
  vapi_type_punt_reason reason; 
} vapi_payload_punt_reason_details;

typedef struct __attribute__ ((__packed__)) {
  vapi_type_msg_header1_t header;
  vapi_payload_punt_reason_details payload;
} vapi_msg_punt_reason_details;

static inline void vapi_msg_punt_reason_details_payload_hton(vapi_payload_punt_reason_details *payload)
{
  vapi_type_punt_reason_hton(&payload->reason);
}

static inline void vapi_msg_punt_reason_details_payload_ntoh(vapi_payload_punt_reason_details *payload)
{
  vapi_type_punt_reason_ntoh(&payload->reason);
}

static inline void vapi_msg_punt_reason_details_hton(vapi_msg_punt_reason_details *msg)
{
  VAPI_DBG("Swapping `vapi_msg_punt_reason_details'@%p to big endian", msg);
  vapi_type_msg_header1_t_hton(&msg->header);
  vapi_msg_punt_reason_details_payload_hton(&msg->payload);
}

static inline void vapi_msg_punt_reason_details_ntoh(vapi_msg_punt_reason_details *msg)
{
  VAPI_DBG("Swapping `vapi_msg_punt_reason_details'@%p to host byte order", msg);
  vapi_type_msg_header1_t_ntoh(&msg->header);
  vapi_msg_punt_reason_details_payload_ntoh(&msg->payload);
}

static inline uword vapi_calc_punt_reason_details_msg_size(vapi_msg_punt_reason_details *msg)
{
  return sizeof(*msg);
}

static void __attribute__((constructor)) __vapi_constructor_punt_reason_details()
{
  static const char name[] = "punt_reason_details";
  static const char name_with_crc[] = "punt_reason_details_aa3dd9e5";
  static vapi_message_desc_t __vapi_metadata_punt_reason_details = {
    name,
    sizeof(name) - 1,
    name_with_crc,
    sizeof(name_with_crc) - 1,
    true,
    offsetof(vapi_type_msg_header1_t, context),
    offsetof(vapi_msg_punt_reason_details, payload),
    sizeof(vapi_msg_punt_reason_details),
    (generic_swap_fn_t)vapi_msg_punt_reason_details_hton,
    (generic_swap_fn_t)vapi_msg_punt_reason_details_ntoh,
    VAPI_INVALID_MSG_ID,
  };

  vapi_msg_id_punt_reason_details = vapi_register_msg(&__vapi_metadata_punt_reason_details);
  VAPI_DBG("Assigned msg id %d to punt_reason_details", vapi_msg_id_punt_reason_details);
}

static inline void vapi_set_vapi_msg_punt_reason_details_event_cb (
  struct vapi_ctx_s *ctx, 
  vapi_error_e (*callback)(struct vapi_ctx_s *ctx, void *callback_ctx, vapi_payload_punt_reason_details *payload),
  void *callback_ctx)
{
  vapi_set_event_cb(ctx, vapi_msg_id_punt_reason_details, (vapi_event_cb)callback, callback_ctx);
};
#endif

#ifndef defined_vapi_msg_punt_reason_dump
#define defined_vapi_msg_punt_reason_dump
typedef struct __attribute__ ((__packed__)) {
  vapi_type_punt_reason reason; 
} vapi_payload_punt_reason_dump;

typedef struct __attribute__ ((__packed__)) {
  vapi_type_msg_header2_t header;
  vapi_payload_punt_reason_dump payload;
} vapi_msg_punt_reason_dump;

static inline void vapi_msg_punt_reason_dump_payload_hton(vapi_payload_punt_reason_dump *payload)
{
  vapi_type_punt_reason_hton(&payload->reason);
}

static inline void vapi_msg_punt_reason_dump_payload_ntoh(vapi_payload_punt_reason_dump *payload)
{
  vapi_type_punt_reason_ntoh(&payload->reason);
}

static inline void vapi_msg_punt_reason_dump_hton(vapi_msg_punt_reason_dump *msg)
{
  VAPI_DBG("Swapping `vapi_msg_punt_reason_dump'@%p to big endian", msg);
  vapi_type_msg_header2_t_hton(&msg->header);
  vapi_msg_punt_reason_dump_payload_hton(&msg->payload);
}

static inline void vapi_msg_punt_reason_dump_ntoh(vapi_msg_punt_reason_dump *msg)
{
  VAPI_DBG("Swapping `vapi_msg_punt_reason_dump'@%p to host byte order", msg);
  vapi_type_msg_header2_t_ntoh(&msg->header);
  vapi_msg_punt_reason_dump_payload_ntoh(&msg->payload);
}

static inline uword vapi_calc_punt_reason_dump_msg_size(vapi_msg_punt_reason_dump *msg)
{
  return sizeof(*msg);
}

static inline vapi_msg_punt_reason_dump* vapi_alloc_punt_reason_dump(struct vapi_ctx_s *ctx)
{
  vapi_msg_punt_reason_dump *msg = NULL;
  const size_t size = sizeof(vapi_msg_punt_reason_dump);
  /* cast here required to play nicely with C++ world ... */
  msg = (vapi_msg_punt_reason_dump*)vapi_msg_alloc(ctx, size);
  if (!msg) {
    return NULL;
  }
  msg->header.client_index = vapi_get_client_index(ctx);
  msg->header.context = 0;
  msg->header._vl_msg_id = vapi_lookup_vl_msg_id(ctx, vapi_msg_id_punt_reason_dump);

  return msg;
}

static inline vapi_error_e vapi_punt_reason_dump(struct vapi_ctx_s *ctx,
  vapi_msg_punt_reason_dump *msg,
  vapi_error_e (*callback)(struct vapi_ctx_s *ctx,
                           void *callback_ctx,
                           vapi_error_e rv,
                           bool is_last,
                           vapi_payload_punt_reason_details *reply),
  void *callback_ctx)
{
  if (!msg || !callback) {
    return VAPI_EINVAL;
  ...
  u32 req_context = vapi_gen_req_context(ctx);
  msg->header.context = req_context;
  vapi_msg_punt_reason_dump_hton(msg);
  if (VAPI_OK == (rv = vapi_send_with_control_ping (ctx, msg, req_context))) {
    vapi_store_request(ctx, req_context, true, (vapi_cb_t)callback, callback_ctx);
    if (VAPI_OK != vapi_producer_unlock (ctx)) {
      abort (); /* this really shouldn't happen */
    ...
    vapi_msg_punt_reason_dump_ntoh(msg);
    if (VAPI_OK != vapi_producer_unlock (ctx)) {
      abort (); /* this really shouldn't happen */
    }
  }
...
static void __attribute__((constructor)) __vapi_constructor_punt_reason_dump()
{
  static const char name[] = "punt_reason_dump";
  static const char name_with_crc[] = "punt_reason_dump_bf8f10da";
  static vapi_message_desc_t __vapi_metadata_punt_reason_dump = {
    name,
    sizeof(name) - 1,
    name_with_crc,
    sizeof(name_with_crc) - 1,
    true,
    offsetof(vapi_type_msg_header2_t, context),
    offsetof(vapi_msg_punt_reason_dump, payload),
    sizeof(vapi_msg_punt_reason_dump),
    (generic_swap_fn_t)vapi_msg_punt_reason_dump_hton,
    (generic_swap_fn_t)vapi_msg_punt_reason_dump_ntoh,
    VAPI_INVALID_MSG_ID,
  };

  vapi_msg_id_punt_reason_dump = vapi_register_msg(&__vapi_metadata_punt_reason_dump);
  VAPI_DBG("Assigned msg id %d to punt_reason_dump", vapi_msg_id_punt_reason_dump);
}
#endif
